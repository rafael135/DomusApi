...
public class CreateTransactionHandler : IRequestHandler<CreateTransactionCommand, CreateTransactionResult>
{
    ...
    public CreateTransactionHandler(DomusDbContext dbContext, ILogger<CreateTransactionHandler> logger)
    {
    ...
    public async Task<CreateTransactionResult> Handle(CreateTransactionCommand request, CancellationToken cancellationToken)
    {
        ...
        TransactionCategory? category = await _dbContext.TransactionCategories.FindAsync(new object[] { request.CategoryId }, cancellationToken);
        if (category is null)
            throw new NotFoundException(nameof(TransactionCategory), request.CategoryId);

        Transaction transaction = Transaction.Create(request.Description, request.Value, request.Type, category, user);
        ...
        return new CreateTransactionResult(new TransactionDto(
            transaction.Id,
